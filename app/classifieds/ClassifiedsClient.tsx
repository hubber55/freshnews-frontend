          ...
          {filteredClassifieds.map((item) => {
                  ...
                  <div>
                    <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                      <div className="flex flex-wrap items-center gap-2 text-[11px] font-bold uppercase tracking-wide">
                        <span className="rounded-full bg-[#00ffff]/10 px-2.5 py-1 text-[#00ffff]">
                          {item.ad_categories?.name || 'Classified'}
                        </span>
                        {item.ad_subcategories?.name ? (
                          <span className="rounded-full bg-white/5 px-2.5 py-1 text-[var(--text-secondary)]">{item.ad_subcategories.name}</span>
                        ) : null}
                      </div>
                      {item.price ? (
                        <div className="flex items-center gap-1.5 rounded-lg bg-[#ffd42a] px-3 py-1 text-[13px] font-black text-black shadow-[0_0_15px_rgba(255,212,42,0.3)]">
                          <Banknote size={14} />
                          {item.price}
                        </div>
                      ) : null}
                    ...
                    <h2 className="text-2xl font-extrabold leading-tight text-white mb-3 group-hover:text-[#00ffff] transition-colors">
                      {item.title}
                    </h2>
                    ...
                    {item.content ? (
                      <p className="whitespace-pre-line text-[14px] leading-6 text-[var(--text-secondary)] line-clamp-3">
                        {item.content}
                      </p>
                    ) : null}
            ...
            return (
              <Link 
                ...
                className="block overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] hover:border-[#00ffff]/50 transition-all duration-300 group shadow-lg"
              >
                {CardContent}
              </Link>
            );
